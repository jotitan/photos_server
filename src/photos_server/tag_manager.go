...
func (tm *TagManager) UpdateExistingPath(pathFrom, pathTo string) {
	if list, exist := tm.TagsByFolder[pathFrom]; exist {
		tm.TagsByFolder[pathTo] = list
		delete(tm.TagsByFolder, pathFrom)
	}
}
